...
public class SimilarNameContactsAdapter extends BaseAdapter {

    private List<String> mSimilarNameContactsDetailsList;
    private Map<String, List<ContactDetailsModel>> mSimilarNameContactsMap = null;
    private static LayoutInflater inflater = null;
    private Context mContext;
    private final ColorGenerator mGenerator;

    public SimilarNameContactsAdapter(Context context, List<String> keyList, Map<String, List<ContactDetailsModel>> SimilarNameContactsMap) {
        this.mSimilarNameContactsDetailsList = keyList;
        this.mContext = context;

        mSimilarNameContactsMap = SimilarNameContactsMap;
    ...
    @Override
    public int getCount() {
        return mSimilarNameContactsDetailsList.size();
    }

    @Override
    public String getItem(int position) {
        return mSimilarNameContactsDetailsList.get(position);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final View view;
        view = inflater.inflate(R.layout.single_similar_name_contacts_view, null);
        holder.mContactIV = (ImageView) view.findViewById(R.id.ivsimilar_name_contacts_contact_image);
        holder.mMainContactNameTV = (TextView) view.findViewById(R.id.tvsimilar_name_contacts_contact_name_m);
        holder.mContactNameTV = (TextView) view.findViewById(R.id.tvsimilar_name_contacts_contact_name);
        holder.mContactCountTV = (TextView) view.findViewById(R.id.count_similar_name_contacts_tv);
        holder.mMainLL = (LinearLayout) view.findViewById(R.id.main_layout);
        holder.view =view;

        String title = mSimilarNameContactsDetailsList.get(position);
        String name = mSimilarNameContactsDetailsList.get(position);
        name = name.substring(0, 1).toUpperCase() + name.substring(1).toLowerCase();
        holder.mContactNameTV.setText(title);
        holder.mMainContactNameTV.setText(name);
        List<ContactDetailsModel> contactDetailsModelList = mSimilarNameContactsMap.get(title);
